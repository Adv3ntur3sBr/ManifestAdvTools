...
local function copy_webkit_files()
    ...
    local js_name = config.WEB_UI_JS_FILE or "manifestadvtools.js"
    local src_js = fs.join(public_dir, js_name)
    if not fs.exists(src_js) then
        src_js = fs.join(public_dir, "luatools.js")
    end
    if fs.exists(src_js) then
        local content = m_utils.read_file(src_js)
        if content then
            m_utils.write_file(fs.join(target_webkit_dir, "manifestadvtools.js"), content)
            m_utils.write_file(fs.join(target_webkit_dir, "luatools.js"), content)
        end
    end
...
local function inject_webkit_files()
    -- Note: webkit/luatools.js is now loaded via .millennium/Dist/webkit.js which bypasses Steam's CSP.
    -- Calling add_browser_js injects <script src="https://millennium.host/..."> which is blocked by Steam's CSP.
    if millennium.add_browser_css then
        pcall(millennium.add_browser_css, "webkit/steamdb-webkit.css")
    end
end
...
local function on_load()
    ...
    pcall(copy_webkit_files)
    pcall(inject_webkit_files)

    local ok_a, res = pcall(api_manifest.init_apis)
    if ok_a and res then
...
-- ── OnlineFix Bundle Download ─────────────────────────────────────────────────

-- Starts a combined manifest + OnlineFix download for the given appid.
-- JS passes: { apiName, appid, contentScriptQuery, installPath, manifestUrl }
-- (Millennium IPC sorts keys alphabetically → positional: apiName, appid, contentScriptQuery, installPath, manifestUrl)
function StartAddWithOnlineFix(apiName, appid, contentScriptQuery, installPath, manifestUrl)
    logger.log("StartAddWithOnlineFix: appid=" .. tostring(appid) ..
               " url=" .. tostring(manifestUrl) .. " api=" .. tostring(apiName))
    local ok, res = pcall(downloads.start_add_with_online_fix,
        appid, manifestUrl, apiName, installPath)
    if not ok then
        logger.warn("StartAddWithOnlineFix CRASHED: " .. tostring(res))
        return json_err(res)
    end
    return json_ok(res)
end

-- Polls the combined manifest+fix download progress.
function GetBundleStatus(appid)
    if type(appid) == "table" then appid = appid.appid end
    local ok, res = pcall(downloads.get_bundle_status, tonumber(appid))
    if not ok then return json_err(res) end
    return json_ok(res)
end
...
function GetSettingsConfig(contentScriptQuery)
...
local exported = {
    ...
    AddCustomApi               = AddCustomApi,
    GetAllApis                 = GetAllApis,
    ToggleApi                  = ToggleApi,
    RemoveApi                  = RemoveApi,
    RenameApi                  = RenameApi,
    ReorderApis                = ReorderApis,
    ...
    StartAddWithOnlineFix      = StartAddWithOnlineFix,
    GetBundleStatus            = GetBundleStatus,
    UninstallFix               = UninstallFix,
    UnFixGame                  = UnFixGame,
...
setmetatable(exported, { __index = _G })
return exported
