...
(function () {
  ...
  function backendLog(message) {
    try {
      if (
        ...
        Millennium.callServerMethod("ManifestAdvTools", "Logger.log", {
          message: String(message),
        });
      }
    } catch (err) {
      if (typeof console !== "undefined" && console.warn) {
        console.warn("[ADV Tools] backendLog failed", err);
      }
    }
  }

  backendLog("ADV Tools script loaded");
  backendLog(
    "Mode Detection: " +
  ...
  function fetchGamesDatabase() {
    ...
    return Millennium.callServerMethod("ManifestAdvTools", "GetGamesDatabase", {
      contentScriptQuery: "",
    })
      .then(function (res) {
      ...
      .catch(function (err) {
        console.warn("[ADV Tools] Failed to fetch games database", err);
        return {};
      });
  }
  ...
  function fetchFixes(appid) {
    ...
    return Millennium.callServerMethod("ManifestAdvTools", "CheckForFixes", {
      appid: appid,
      contentScriptQuery: "",
    })
      .then(function (res) {
      ...
      .catch(function (err) {
        console.warn("[ADV Tools] Failed to fetch fixes", err);
        return null;
      });
  }
  ...
  const DEFAULT_THEMES = {
    original: {
      ...
      bgPrimary: "#0d0f1a",
      bgSecondary: "#161929",
      bgTertiary: "rgba(30, 34, 60, 0.92)",
      bgHover: "rgba(55, 60, 110, 0.92)",
      bgContainer: "rgba(25, 28, 55, 0.72)",
      bgContainerGradient: "rgba(25, 28, 55, 0.90), #07080f",
      accent: "#a78bfa",
      accentLight: "#c4b5fd",
      accentDark: "#7c3aed",
      border: "rgba(167,139,250,0.32)",
      borderHover: "rgba(167,139,250,0.85)",
      text: "#f1f0ff",
      textSecondary: "#b8b5d8",
      gradient: "linear-gradient(135deg, #7c3aed 0%, #a78bfa 50%, #c4b5fd 100%)",
      gradientLight: "linear-gradient(135deg, #a78bfa 0%, #c4b5fd 100%)",
      shadow: "rgba(124,58,237,0.45)",
      shadowHover: "rgba(167,139,250,0.65)",
    },
  };
  ...
  function loadThemesFromBackend() {
    ...
    return Millennium.callServerMethod("ManifestAdvTools", "GetThemes", {
      contentScriptQuery: "",
    })
      .then(function (res) {
        try {
  ...
  function generateThemeStyles(theme) {
    return `
            ...
            /* Store header button - ADV Tools themed icon button */
  ...
  function ensureThemeStylesheet(themeKey) {
    ...
    try {
      ...
      backendLog("ManifestAdvTools: Theme CSS injection failed: " + err);
    }
  }

  function ensureLuaToolsStyles() {
    /* ADV_TOOLS_PREMIUM_CSS_INJECTED */
    const themeKey = getCurrentThemeKey();
    const theme = getCurrentTheme();
    const colors = getThemeColors();

    try {
      const advStyleId = "adv-tools-premium-styles";
      let advStyle = document.getElementById(advStyleId);
      if (!advStyle) {
        advStyle = document.createElement("style");
        advStyle.id = advStyleId;
      ...
      advStyle.textContent = `
        ...
        :root {
          --lt-accent: ${colors.accent};
          --lt-accent-light: ${colors.accentLight};
          --lt-bg-primary: ${theme.bgPrimary};
          --lt-bg-secondary: ${theme.bgSecondary};
          --lt-bg-tertiary: ${theme.bgTertiary};
          --lt-border: ${colors.borderRgba};
          --lt-text: ${colors.text};
          --lt-text-sec: ${colors.textSecondary};
          --lt-gradient: ${colors.gradient};
          --lt-gradient-light: ${colors.gradientLight};
          --lt-shadow: ${colors.shadow};
          --lt-rgb: ${colors.rgbString};
        }

        /* ADV Tools Premium Theme - Adaptive to Active Theme */
        .luatools-modal, .luatools-overlay > div, .luatools-settings-overlay > div,
        .luatools-fixes-results-overlay > div, .luatools-loadedapps-overlay > div,
        .luatools-settings-manager-overlay > div, .luatools-unfix-overlay > div,
        .luatools-custom-api-overlay > div, .luatools-alert-overlay > div,
        .luatools-confirm-overlay > div {
          font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Motiva Sans', sans-serif !important;
          border-radius: 16px !important;
          border: 1px solid rgba(${colors.rgbString}, 0.28) !important;
          box-shadow: 0 20px 60px rgba(0,0,0,0.65), 0 0 28px rgba(${colors.rgbString}, 0.18) !important;
          background: linear-gradient(145deg, ${theme.bgPrimary} 0%, ${theme.bgSecondary} 100%) !important;
          color: ${colors.text} !important;
        }

        /* Premium header gradient bar */
        .luatools-settings-overlay .luatools-modal-title,
        .luatools-overlay .luatools-modal-title,
        .luatools-header-title {
          background: ${colors.gradientLight} !important;
        ...
        .luatools-button-primary, .luatools-btn-primary,
        button.luatools-button[style*="background"] {
          background: ${colors.gradient} !important;
          border: 1px solid rgba(${colors.rgbString}, 0.45) !important;
          box-shadow: 0 4px 14px rgba(${colors.rgbString}, 0.35), 0 2px 6px rgba(0,0,0,0.4) !important;
        ...
        .luatools-button-primary:hover, .luatools-btn-primary:hover {
          box-shadow: 0 6px 22px rgba(${colors.rgbString}, 0.55), 0 4px 12px rgba(0,0,0,0.5) !important;
          transform: translateY(-1px) !important;
        }

        /* ADV Tools logo header in the main menu */
        .luatools-menu-title-icon, .luatools-title-icon {
          filter: drop-shadow(0 0 8px rgba(${colors.rgbString}, 0.6)) !important;
        }

        /* Glowing overlay backdrops */
        .luatools-overlay, .luatools-settings-overlay,
        .luatools-fixes-results-overlay, .luatools-loadedapps-overlay,
        .luatools-settings-manager-overlay, .luatools-unfix-overlay,
        .luatools-custom-api-overlay, .luatools-alert-overlay,
        .luatools-confirm-overlay {
        ...
        .luatools-progress-bar-fill, [class*="progress-fill"] {
          background: ${colors.gradient} !important;
          background-size: 200% 100% !important;
          animation: advProgressShimmer 1.8s ease infinite !important;
          border-radius: 4px !important;
        }

        @keyframes advProgressShimmer {
          0%   { background-position: 100% 0; }
          100% { background-position: -100% 0; }
        }

        /* Input fields */
        .luatools-overlay input, .luatools-settings-overlay input,
        .luatools-settings-manager-overlay input {
          background: rgba(${colors.rgbString}, 0.07) !important;
          border: 1px solid rgba(${colors.rgbString}, 0.25) !important;
          border-radius: 8px !important;
          color: ${colors.text} !important;
          transition: border-color 0.2s ease, box-shadow 0.2s ease !important;
        }
        .luatools-overlay input:focus, .luatools-settings-overlay input:focus,
        .luatools-settings-manager-overlay input:focus {
          border-color: ${colors.accent} !important;
          box-shadow: 0 0 0 3px rgba(${colors.rgbString}, 0.2) !important;
          outline: none !important;
        }

        /* Section cards */
        .luatools-section, [class*="luatools-card"], [class*="luatools-group"] {
          background: rgba(${colors.rgbString}, 0.04) !important;
          border: 1px solid rgba(${colors.rgbString}, 0.16) !important;
          border-radius: 12px !important;
        }

        /* Scrollbar styling */
        .luatools-overlay *::-webkit-scrollbar,
        .luatools-settings-manager-overlay *::-webkit-scrollbar { width: 6px; height: 6px; }
        .luatools-overlay *::-webkit-scrollbar-track,
        .luatools-settings-manager-overlay *::-webkit-scrollbar-track { background: rgba(0,0,0,0.2); border-radius: 3px; }
        .luatools-overlay *::-webkit-scrollbar-thumb,
        .luatools-settings-manager-overlay *::-webkit-scrollbar-thumb {
          background: rgba(${colors.rgbString}, 0.35);
          border-radius: 3px;
        }
        .luatools-overlay *::-webkit-scrollbar-thumb:hover,
        .luatools-settings-manager-overlay *::-webkit-scrollbar-thumb:hover {
          background: rgba(${colors.rgbString}, 0.65);
        }

        /* "Add via ADV Tools" button in Steam store page */
        .luatools-store-btn, [id*="luatools-header-btn"], [id*="luatools-store"], .luatools-bundle-btn {
          background: ${colors.gradient} !important;
          border: 1px solid rgba(${colors.rgbString}, 0.5) !important;
          box-shadow: 0 0 12px rgba(${colors.rgbString}, 0.35) !important;
          border-radius: 6px !important;
          font-weight: 600 !important;
          transition: all 0.2s ease !important;
          color: #ffffff !important;
        }
        .luatools-store-btn:hover, [id*="luatools-header-btn"]:hover, [id*="luatools-store"]:hover, .luatools-bundle-btn:hover {
          box-shadow: 0 0 20px rgba(${colors.rgbString}, 0.6) !important;
          transform: translateY(-1px) !important;
        }

        /* ADV Tools badge / tag in overlays */
        .luatools-badge, [class*="luatools-tag"], [class*="luatools-label"] {
          background: rgba(${colors.rgbString}, 0.16) !important;
          border: 1px solid rgba(${colors.rgbString}, 0.3) !important;
          border-radius: 20px !important;
          padding: 2px 10px !important;
          font-size: 11px !important;
          font-weight: 600 !important;
          color: ${colors.accentLight} !important;
        }

        /* ADV Tools tab-style navigation */
        .luatools-tab-bar {
          display: flex !important;
          gap: 4px !important;
          padding: 0 20px !important;
          background: rgba(0, 0, 0, 0.25) !important;
          border-bottom: 1px solid rgba(${colors.rgbString}, 0.15) !important;
          overflow-x: auto !important;
          scrollbar-width: none !important;
        }
        .luatools-tab-bar::-webkit-scrollbar { display: none; }
        .luatools-tab-btn {
        ...
        .luatools-tab-btn:hover {
          color: ${colors.text} !important;
          background: rgba(${colors.rgbString}, 0.08) !important;
        }
        .luatools-tab-btn.active {
          color: ${colors.accent} !important;
          border-bottom-color: ${colors.accent} !important;
          background: rgba(${colors.rgbString}, 0.12) !important;
        }
        .luatools-tab-badge {
          background: rgba(${colors.rgbString}, 0.25) !important;
          color: ${colors.text} !important;
    ...
    if (styleEl) {
      ...
      try {
        ...
        backendLog("ManifestAdvTools: Styles injection failed: " + err);
      }
    }
  }
  ...
  function ensureFontAwesome() {
    ...
    try {
      ...
      backendLog("ManifestAdvTools: Font Awesome injection failed: " + err);
    }
  }
  ...
  function showCustomApiModal(onSuccess) {
    ...
    saveBtn.onclick = function () {
      ...
      Millennium.callServerMethod("ManifestAdvTools", "AddCustomApi", {
        name: name,
        url: url,
    ...
    ensureTranslationsLoaded(false)
      ...
      .finally(function () {
        ...
        function createCardButton(id, key, fallback, iconClass) {
          ...
          btn.href = "#";
          const btnColors = getThemeColors();
          btn.style.cssText = `display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;background:rgba(${btnColors.rgbString},0.06);border:1px solid ${btnColors.borderRgba};border-radius:12px;color:${btnColors.text};font-size:12px;font-weight:600;text-decoration:none;transition:all 0.2s cubic-bezier(0.2, 0.8, 0.2, 1);cursor:pointer;text-align:center;padding:16px 10px;box-shadow:0 2px 8px rgba(0,0,0,0.2);min-width:0;`;
          const iconHtml = iconClass
            ...
            : "";
          const textSpan =
            '<span style="text-align:center;line-height:1.3;letter-spacing:0.2px;">' +
            t(key, fallback) +
            "</span>";
          btn.innerHTML = iconHtml + textSpan;
          btn.onmouseover = function () {
            const c = getThemeColors();
            this.style.background = `rgba(${c.rgbString},0.16)`;
            this.style.transform = "translateY(-2px)";
            this.style.boxShadow = `0 6px 18px ${c.shadowRgba}`;
        ...
        removeBtn.onmouseout = function () {
          this.style.background = "rgba(255,80,80,0.08)";
        ...
        cardGrid.style.cssText = isGamePage
        ...
        cardGrid.appendChild(restartSteamCardBtn);
        ...
        if (checkBtn) {
          checkBtn.addEventListener("click", function (e) {
            ...
            try {
              Millennium.callServerMethod("ManifestAdvTools", "CheckForUpdatesNow", {
                contentScriptQuery: "",
              }).then(function (res) {
                try {
                  const payload =
        ...
        if (typeof restartSteamCardBtn !== "undefined" && restartSteamCardBtn) {
          restartSteamCardBtn.addEventListener("click", function (e) {
            e.preventDefault();
        ...
        if (fetchApisBtn) {
          fetchApisBtn.addEventListener("click", function (e) {
            ...
            try {
              Millennium.callServerMethod("ManifestAdvTools", "FetchFreeApisNow", {
                contentScriptQuery: "",
              }).then(function (res) {
                try {
                  const payload =
        ...
        if (fixesMenuBtn) {
          fixesMenuBtn.addEventListener("click", function (e) {
            ...
            try {
              ...
              Millennium.callServerMethod("ManifestAdvTools", "GetGameInstallPath", {
                appid,
                contentScriptQuery: "",
              })
                .then(function (pathRes) {
                  try {
                    ...
                    backendLog("ManifestAdvTools: GetGameInstallPath error: " + err);
                    try {
                      overlay.remove();
                    } catch (_) {}
                  }
        ...
        try {
          ...
          if (
            ...
            Millennium.callServerMethod("ManifestAdvTools", "HasLuaToolsForApp", {
              appid,
              contentScriptQuery: "",
            }).then(function (res) {
              try {
                ...
                if (exists) {
                  const doDelete = function () {
                    try {
                      Millennium.callServerMethod("ManifestAdvTools",
                  ...
                  removeBtn.onclick = function (e) {
                    ...
                    const confirmMessage = t(
                      "menu.remove.confirm",
                      "Remove via ADV Tools for this game?",
                    );
                    showLuaToolsConfirm(
                      "ManifestAdvTools",
                      confirmMessage,
  ...
  function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
    try {
      ...
      return Millennium.callServerMethod("ManifestAdvTools", "GetTranslations", {
        ...
        .catch(function (err) {
          backendLog("ManifestAdvTools: translation load failed: " + err);
          window.__LuaToolsI18n = window.__LuaToolsI18n || {
            language: "en",
            locales: [],
            strings: {},
      ...
      backendLog("ManifestAdvTools: ensureTranslationsLoaded error: " + err);
      window.__LuaToolsI18n = window.__LuaToolsI18n || {
        language: "en",
        locales: [],
        strings: {},
  ...
  function askRestartConfirmation() {
    showLuaToolsConfirm(
      ...
      function () {
        try {
          Millennium.callServerMethod("ManifestAdvTools", "RestartSteam", {
            contentScriptQuery: "",
          });
  ...
  function showTestPopup() {
    ...
    if (
      ...
      Millennium.callServerMethod("ManifestAdvTools", "GetApiList", {
        contentScriptQuery: "",
      })
        .then(function (res) {
          try {
    ...
    function cancelOperation() {
      ...
      try {
        ...
        if (
          ...
          Millennium.callServerMethod("ManifestAdvTools", "CancelAddViaLuaTools", {
            appid,
            contentScriptQuery: "",
          });
        }
  ...
  function showFixesResultsPopup(data, isGameInstalled) {
    ...
    gameFolderBtn.onclick = function (e) {
      ...
      if (window.__LuaToolsGameInstallPath) {
        try {
          Millennium.callServerMethod("ManifestAdvTools", "OpenGameFolder", {
            path: window.__LuaToolsGameInstallPath,
            contentScriptQuery: "",
          });
        } catch (err) {
          backendLog("ManifestAdvTools: Failed to open game folder: " + err);
        }
      }
    };
    ...
    function startUnfix(appid) {
      try {
        Millennium.callServerMethod("ManifestAdvTools", "UnFixGame", {
          appid: appid,
  ...
  function applyFix(appid, downloadUrl, fixType, gameName, resultsOverlay) {
    try {
      ...
      backendLog("ManifestAdvTools: Applying fix " + fixType + " for appid " + appid);

      // Start the download and extraction process
      Millennium.callServerMethod("ManifestAdvTools", "ApplyGameFix", {
        appid: appid,
        ...
        .then(function (res) {
          try {
            const payload = typeof res === "string" ? JSON.parse(res) : res;
            if (payload && payload.success) {
              // Show download progress popup similar to Add via ADV Tools
  ...
  function showFixDownloadProgress(appid, fixType) {
    // Reuse the download popup UI from Add via ADV Tools
    if (document.querySelector(".luatools-overlay")) return;
    ...
    cancelBtn.onclick = function (e) {
      ...
      Millennium.callServerMethod("ManifestAdvTools", "CancelApplyFix", {
        appid: appid,
        contentScriptQuery: "",
      })
        .then(function (res) {
  ...
  function pollFixProgress(appid, fixType) {
    const poll = function () {
      try {
        ...
        Millennium.callServerMethod("ManifestAdvTools", "GetApplyFixStatus", {
          appid: appid,
          contentScriptQuery: "",
        }).then(function (res) {
          try {
            ...
            backendLog("ManifestAdvTools: GetApplyFixStatus error: " + err);
          }
        });
      } catch (err) {
        backendLog("ManifestAdvTools: pollFixProgress error: " + err);
      }
    };
    setTimeout(poll, 500);
  }
  ...
  function pollUnfixProgress(appid) {
    const poll = function () {
      try {
        ...
        Millennium.callServerMethod("ManifestAdvTools", "GetUnfixStatus", {
          appid: appid,
          contentScriptQuery: "",
        }).then(function (res) {
          try {
              ...
              if (state.status === "removing") {
                ...
                setTimeout(function () {
                  try {
                    ...
                    backendLog("ManifestAdvTools: Running verify for appid " + appid);
                  } catch (_) {}
                }, 1000);
            ...
            backendLog("ManifestAdvTools: GetUnfixStatus error: " + err);
          }
        });
      } catch (err) {
        backendLog("ManifestAdvTools: pollUnfixProgress error: " + err);
      }
    };
    setTimeout(poll, 500);
  }
  ...
  function showSettingsManagerPopup(forceRefresh, onBack) {
    ...
    tabBar.className = "luatools-tab-bar";
    ...
    TABS.forEach(function (tabDef) {
      ...
      btn.type = "button";
      btn.className = "luatools-tab-btn" + (tabDef.id === "general" ? " active" : "");
      btn.dataset.tabId = tabDef.id;
      btn.innerHTML = `<i class="fa-solid ${tabDef.icon}"></i><span>${t(tabDef.labelKey, tabDef.labelFallback)}</span>`;
    ...
    searchWrap.style.cssText = `display:flex;align-items:center;gap:10px;padding:9px 14px;background:${searchColors.bgTertiary};border:1px solid ${searchColors.border};border-radius:10px;transition:all 0.2s ease;`;
    ...
    searchInput.placeholder = t(
      ...
      "Pesquisar configurações, opções e preferências...",
    );
    searchInput.style.cssText = `flex:1;background:transparent;border:none;outline:none;color:${searchColors.text};font-size:13px;`;
    ...
    searchClear.title = t("settings.search.clear", "Limpar busca");
    ...
    panelGeneral.id = "luatools-panel-general";
    panelGeneral.dataset.tabId = "general";

    const panelApis = document.createElement("div");
    panelApis.id = "luatools-panel-apis";
    panelApis.dataset.tabId = "apis";
    panelApis.style.display = "none";

    const panelFixes = document.createElement("div");
    panelFixes.id = "luatools-panel-fixes";
    panelFixes.dataset.tabId = "fixes";
    panelFixes.style.display = "none";

    const panelLuas = document.createElement("div");
    panelLuas.id = "luatools-panel-luas";
    panelLuas.dataset.tabId = "luas";
    panelLuas.style.display = "none";

    const panelAbout = document.createElement("div");
    panelAbout.id = "luatools-panel-about";
    panelAbout.dataset.tabId = "about";
    panelAbout.style.display = "none";

    contentWrap.appendChild(panelGeneral);
    contentWrap.appendChild(panelApis);
    contentWrap.appendChild(panelFixes);
    contentWrap.appendChild(panelLuas);
    contentWrap.appendChild(panelAbout);
    ...
    if (window.__LUATOOLS_IS_BIG_PICTURE__) {
      ...
      tip.innerHTML =
        ...
        t(
          "bigpicture.mouseTip",
          "Para usar o mouse na Steam: Botão Guia + Analógico Direito, clique com RB",
        );
      contentWrap.insertBefore(tip, contentWrap.firstChild);
    }
    ...
    function switchTab(tabId) {
      ...
      panelGeneral.style.display = tabId === "general" ? "block" : "none";
      panelApis.style.display = tabId === "apis" ? "block" : "none";
      panelFixes.style.display = tabId === "fixes" ? "block" : "none";
      panelLuas.style.display = tabId === "luas" ? "block" : "none";
      panelAbout.style.display = tabId === "about" ? "block" : "none";

      if (tabId === "general") {
        searchInput.placeholder = t("settings.search.general", "Pesquisar preferências e opções gerais...");
      } else if (tabId === "apis") {
        searchInput.placeholder = t("settings.search.apis", "Pesquisar fontes de download...");
      } else if (tabId === "fixes") {
        searchInput.placeholder = t("settings.search.fixes", "Pesquisar correções por jogo ou AppID...");
      } else if (tabId === "luas") {
      ...
      applySearchFilter();
      if (window.GamepadNav && typeof window.GamepadNav.scanElements === "function") {
        setTimeout(window.GamepadNav.scanElements, 100);
      }
    }
    ...
    function applySearchFilter() {
      ...
      if (tab === "general") {
        // Filter settings options in panelGeneral
        const optionEls = panelGeneral.querySelectorAll("[data-setting-option]");
        ...
        const groupEls = panelGeneral.querySelectorAll("[data-setting-group]");
        ...
        const apiRows = panelApis.querySelectorAll("[data-api-name]");
        apiRows.forEach(function (row) {
          const name = (row.dataset.apiName || "").toLowerCase();
    ...
    function applyStaticTranslations() {
      title.textContent = t("settings.title", "ADV Tools · Settings");
    ...
    function renderSettings() {
      ...
      if (
        ...
        panelGeneral.appendChild(emptyState);
        updateSaveState();
        return;
      }
      ...
      for (let i = 0; i < state.config.schema.length; i++) {
            ...
            if (option.type === "select") {
              ...
              selectEl.addEventListener("change", function () {
                ...
                if (group.key === "general" && option.key === "theme") {
                  try {
                    backendLog(
                      "ManifestAdvTools: Theme change detected, new value: " +
                        selectEl.value,
                    );
                  } catch (_) {}
                  ...
                  if (
                    ...
                    try {
                      backendLog(
                        "ManifestAdvTools: Updated cache, theme is now: " +
                          window.__LuaToolsSettings.values.general.theme,
                      );
                    } catch (_) {}
                  }
              ...
              if (option.key === "morrenusApiKey") {
                ...
                const updateStats = function (key) {
                  ...
                  statsDiv.innerHTML =
                    ...
                    lt("Checking key...");
                  Millennium.callServerMethod("ManifestAdvTools", "GetMorrenusStats", {
                    api_key: key,
                    contentScriptQuery: "",
                  })
        ...
        panelGeneral.appendChild(groupEl);
      }

      // Render API Toggles section into panelApis
      renderApiTogglesSection(panelApis);

      // Render Installed Fixes section into panelFixes
      renderInstalledFixesSection(panelFixes);

      // Render Installed Lua Scripts section into panelLuas
      renderInstalledLuaSection(panelLuas);

      // Render About section into panelAbout
      renderAboutSection(panelAbout);

      updateSaveState();
      refreshDependencies();
      switchTab(state.currentTab || "general");
    }
    ...
    function renderInstalledFixesSection(targetParent) {
      const parent = targetParent || panelFixes || contentWrap;
      parent.innerHTML = "";
      const sectionEl = document.createElement("div");
      sectionEl.id = "luatools-installed-fixes-section";
      const sectionColors = getThemeColors();
      sectionEl.style.cssText = `margin-top:0;padding:20px;background:rgba(${sectionColors.rgbString},0.04);border:1px solid ${sectionColors.border};border-radius:10px;`;

      const sectionTitle = document.createElement("div");
      const titleColors = getThemeColors();
      sectionTitle.style.cssText = `font-size:16px;color:${titleColors.text};margin-bottom:14px;font-weight:600;display:flex;align-items:center;gap:8px;`;
      sectionTitle.innerHTML =
        '<i class="fa-solid fa-wrench" style="color:' + titleColors.accent + ';"></i>' +
        t("settings.installedFixes.title", "Correções Instaladas");
    ...
    function loadInstalledFixes(container) {
      ...
      Millennium.callServerMethod("ManifestAdvTools", "GetInstalledFixes", {
        contentScriptQuery: "",
      })
        .then(function (res) {
          const response = typeof res === "string" ? JSON.parse(res) : res;
          backendLog(
            "ManifestAdvTools: GetInstalledFixes response: " +
              JSON.stringify(response).substring(0, 200),
          );
          if (!response || !response.success) {
            backendLog(
              "ManifestAdvTools: GetInstalledFixes failed - response: " +
        ...
        .catch(function (err) {
          backendLog("ManifestAdvTools: GetInstalledFixes catch error: " + err);
    ...
    function renderApiTogglesSection(targetParent) {
      const parent = targetParent || panelApis || contentWrap;
      parent.innerHTML = "";
      const c = getThemeColors();
      const sectionEl = document.createElement("div");
      sectionEl.id = "luatools-api-toggles-section";
      sectionEl.style.cssText = `margin-top:0;padding:20px;background:rgba(${c.rgbString},0.04);border:1px solid ${c.border};border-radius:10px;`;

      const sectionTop = document.createElement("div");
      sectionTop.style.cssText = "display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px;gap:12px;flex-wrap:wrap;";

      const textWrap = document.createElement("div");
      textWrap.style.flex = "1";

      const sectionTitle = document.createElement("div");
      sectionTitle.style.cssText = `font-size:16px;color:${c.text};margin-bottom:4px;font-weight:600;display:flex;align-items:center;gap:8px;`;
      sectionTitle.innerHTML = '<i class="fa-solid fa-plug" style="color:' + c.accent + ';"></i>' + t("settings.apiToggles.title", "Fontes de Download (APIs)");
      textWrap.appendChild(sectionTitle);

      const sectionDesc = document.createElement("div");
      sectionDesc.style.cssText = `font-size:12px;color:${c.textSecondary};line-height:1.4;`;
      sectionDesc.textContent = t("settings.apiToggles.desc", "Ative ou desative as fontes de manifestos. Fontes desativadas serão ignoradas na busca.");
      textWrap.appendChild(sectionDesc);
      sectionTop.appendChild(textWrap);

      const addCustomBtn = document.createElement("a");
      addCustomBtn.href = "#";
      addCustomBtn.style.cssText = `display:inline-flex;align-items:center;gap:6px;padding:8px 14px;background:rgba(${c.rgbString},0.12);border:1px solid ${c.border};border-radius:8px;color:${c.accent};font-size:12px;font-weight:600;text-decoration:none;cursor:pointer;white-space:nowrap;transition:all 0.2s ease;`;
      addCustomBtn.innerHTML = '<i class="fa-solid fa-plus"></i> ' + t("settings.api.addCustom", "Adicionar API");
      addCustomBtn.onmouseover = function() {
        this.style.background = `rgba(${c.rgbString},0.22)`;
        this.style.transform = "translateY(-1px)";
      };
      addCustomBtn.onmouseout = function() {
        this.style.background = `rgba(${c.rgbString},0.12)`;
        this.style.transform = "none";
      };
      addCustomBtn.onclick = function(e) {
        e.preventDefault();
        showCustomApiModal(function() {
          renderApiTogglesSection(parent);
        });
      };
      sectionTop.appendChild(addCustomBtn);
      sectionEl.appendChild(sectionTop);

      const listEl = document.createElement("div");
      listEl.id = "luatools-api-list";
      listEl.innerHTML = `<div style="padding:16px;text-align:center;color:${c.textSecondary};font-size:13px;"><i class="fa-solid fa-spinner fa-spin" style="margin-right:8px;"></i>Carregando fontes...</div>`;
      sectionEl.appendChild(listEl);

      parent.appendChild(sectionEl);

      Millennium.callServerMethod("ManifestAdvTools", "GetAllApis", { contentScriptQuery: "" })
        .then(function(res) {
          const payload = typeof res === "string" ? JSON.parse(res) : res;
          if (!payload || !payload.success || !Array.isArray(payload.apis)) {
            listEl.innerHTML = `<div style="color:#ff5c5c;font-size:13px;padding:10px;">${t("settings.apiToggles.error", "Falha ao carregar APIs.")}</div>`;
            return;
          }
          listEl.innerHTML = "";
          if (payload.apis.length === 0) {
            listEl.innerHTML = `<div style="color:${getThemeColors().textSecondary};font-size:13px;padding:10px;">${t("settings.apiToggles.empty", "Nenhuma API configurada.")}</div>`;
          } else {
          payload.apis.forEach(function(api) {
            ...
            row.addEventListener('dragend', function() {
                ...
                Millennium.callServerMethod("ManifestAdvTools", "ReorderApis", { apiNames: JSON.stringify(newOrder), contentScriptQuery: "" })
                  .then(function(r) {
            ...
            nameDisplay.onclick = function() {
              ...
              function commitRename() {
                ...
                if (newVal && newVal !== currentName) {
                  Millennium.callServerMethod("ManifestAdvTools", "RenameApi", { old_name: currentName, new_name: newVal, contentScriptQuery: "" })
                    .then(function(r) {
                      const rp = typeof r === "string" ? JSON.parse(r) : r;
                      if (rp && rp.success) {
                        currentName = newVal;
            ...
            pill.onclick = function() {
              ...
              Millennium.callServerMethod("ManifestAdvTools", "ToggleApi", { apiName: currentName, contentScriptQuery: "" })
                .then(function(r) {
                  const rp = typeof r === "string" ? JSON.parse(r) : r;
                  if (!rp || !rp.success) {
            ...
            delBtn.onclick = function(e) {
              ...
              Millennium.callServerMethod("ManifestAdvTools", "RemoveApi", { apiName: currentName, contentScriptQuery: "" })
                .then(function(r) {
                  const rp = typeof r === "string" ? JSON.parse(r) : r;
                  if (rp && rp.success) {
                    row.style.opacity = "0";
    ...
    function renderInstalledLuaSection(targetParent) {
      const parent = targetParent || panelLuas || contentWrap;
      parent.innerHTML = "";
      const sectionEl = document.createElement("div");
      sectionEl.id = "luatools-installed-lua-section";
      const sectionLuaColors = getThemeColors();
      sectionEl.style.cssText = `margin-top:0;padding:20px;background:rgba(${sectionLuaColors.rgbString},0.04);border:1px solid ${sectionLuaColors.border};border-radius:10px;`;

      const sectionTitleContainer = document.createElement("div");
      sectionTitleContainer.style.cssText = "display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px;";

      const sectionTitle = document.createElement("div");
      const luaTitleColors = getThemeColors();
      sectionTitle.style.cssText = `font-size:16px;color:${luaTitleColors.text};font-weight:600;display:flex;align-items:center;gap:8px;`;
      sectionTitle.innerHTML =
        '<i class="fa-solid fa-gamepad" style="color:' + luaTitleColors.accent + ';"></i>' +
        t("settings.installedLua.title", "Jogos & Manifestos Instalados");
      ...
      [5, 10, 25, 50, 100].forEach(function(val) {
        ...
        opt.value = val;
        opt.textContent = val + " " + t("settings.perPage", "por página");
    ...
    function loadInstalledLuaScripts(container) {
      ...
      Millennium.callServerMethod("ManifestAdvTools", "GetInstalledLuaScripts", {
        ...
        .then(function (res) {
          const response = typeof res === "string" ? JSON.parse(res) : res;
          if (!response || !response.success) {
            container.innerHTML = `<div style="padding:14px;background:rgba(255,92,92,0.08);border:1px solid rgba(255,92,92,0.3);border-radius:8px;color:#ff5c5c;text-align:center;font-size:13px;">${t("settings.installedLua.error", "Falha ao carregar jogos instalados.")}</div>`;
            return;
          }
    ...
    function createLuaListItem(script, container) {
      ...
      itemEl.style.cssText = `padding:14px 16px;background:rgba(${itemLuaColors.rgbString},0.04);border:1px solid ${itemLuaColors.border};border-radius:10px;display:flex;justify-content:space-between;align-items:center;transition:all 0.2s ease;gap:12px;margin-bottom:8px;`;
      ...
      gameName.style.cssText = `font-size:15px;font-weight:600;color:${gameNameLuaColors.text};margin-bottom:5px;display:flex;align-items:center;flex-wrap:wrap;gap:8px;`;
      gameName.textContent = gameNameText;
      ...
      if (script.isDisabled) {
        ...
        disabledBadge.style.cssText =
          "padding:2px 8px;background:rgba(255,193,7,0.15);border:1px solid rgba(255,193,7,0.4);border-radius:20px;font-size:11px;color:#ffc107;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;";
      ...
      detailsDiv.style.cssText = `font-size:12px;color:${detailsLuaColors.textSecondary};display:flex;flex-wrap:wrap;align-items:center;gap:8px;`;

      // AppID Pill with Copy capability
      const appidSpan = document.createElement("span");
      appidSpan.style.cssText = `display:inline-flex;align-items:center;gap:4px;cursor:pointer;background:rgba(${detailsLuaColors.rgbString},0.08);border:1px solid ${detailsLuaColors.border};padding:2px 8px;border-radius:6px;font-weight:500;transition:all 0.15s;`;
      appidSpan.innerHTML = `<i class="fa-solid fa-hashtag" style="opacity:0.6;"></i><span>${script.appid}</span> <i class="fa-regular fa-copy" style="font-size:10px;opacity:0.7;"></i>`;
      appidSpan.title = "Clique para copiar AppID";
      appidSpan.onclick = function() {
        try {
          navigator.clipboard.writeText(String(script.appid));
          appidSpan.innerHTML = `<i class="fa-solid fa-check" style="color:#4ade80;"></i><span style="color:#4ade80;">Copiado!</span>`;
          setTimeout(() => {
            appidSpan.innerHTML = `<i class="fa-solid fa-hashtag" style="opacity:0.6;"></i><span>${script.appid}</span> <i class="fa-regular fa-copy" style="font-size:10px;opacity:0.7;"></i>`;
          }, 1500);
        } catch(_) {}
      };
      detailsDiv.appendChild(appidSpan);

      // SteamDB link
      const steamdbLink = document.createElement("a");
      steamdbLink.href = "#";
      steamdbLink.style.cssText = `display:inline-flex;align-items:center;gap:4px;color:${detailsLuaColors.accent};text-decoration:none;background:rgba(${detailsLuaColors.rgbString},0.08);border:1px solid ${detailsLuaColors.border};padding:2px 8px;border-radius:6px;font-size:11px;font-weight:500;transition:all 0.15s;cursor:pointer;`;
      steamdbLink.innerHTML = `<i class="fa-solid fa-arrow-up-right-from-square" style="font-size:10px;"></i><span>SteamDB</span>`;
      steamdbLink.title = "Abrir página no SteamDB";
      steamdbLink.onclick = function(e) {
        e.preventDefault();
        try {
          Millennium.callServerMethod("ManifestAdvTools", "OpenExternalUrl", { url: "https://steamdb.info/app/" + script.appid + "/" });
        } catch(_) {}
      };
      detailsDiv.appendChild(steamdbLink);

      if (script.modifiedDate) {
        const dateSpan = document.createElement("span");
        const dateLuaColors = getThemeColors();
        dateSpan.style.cssText = "display:inline-flex;align-items:center;gap:4px;";
        dateSpan.innerHTML = `<i class="fa-solid fa-clock-rotate-left" style="color:${dateLuaColors.accent};opacity:0.6;"></i><span>${script.modifiedDate}</span>`;
        detailsDiv.appendChild(dateSpan);
      }
      ...
      luaDeleteBtn.title = t("settings.installedLua.delete", "Remover");
      ...
      luaDeleteBtn.addEventListener("click", function (e) {
        ...
        showLuaToolsConfirm(
          ...
          function () {
            ...
            Millennium.callServerMethod("ManifestAdvTools", "DeleteLuaToolsForApp", {
              appid: script.appid,
              contentScriptQuery: "",
            })
              .then(function (res) {
                ...
                setTimeout(function () {
                  ...
                  if (container.children.length === 0) {
                    ...
                    container.innerHTML = `<div style="padding:14px;background:${emptyLuaColors.bgTertiary};border:1px solid ${emptyLuaColors.border};border-radius:4px;color:${emptyLuaColors.textSecondary};text-align:center;">${t("settings.installedLua.empty", "Nenhum jogo instalado ainda.")}</div>`;
                  }
                }, 300);
              })
              .catch(function (err) {
    ...
    function renderAboutSection(container) {
      ...
      aboutWrap.style.cssText = "display:flex;flex-direction:column;gap:18px;";

      // Hero Card
      const heroCard = document.createElement("div");
      heroCard.style.cssText = `background:linear-gradient(135deg, rgba(${c.rgbString},0.12) 0%, rgba(${c.rgbString},0.04) 100%);border:1px solid ${c.border};border-radius:14px;padding:22px 24px;display:flex;align-items:center;gap:18px;box-shadow:0 8px 24px rgba(0,0,0,0.25);`;
      ...
      try {
        Millennium.callServerMethod("ManifestAdvTools", "GetIconDataUrl", {}).then(function(res) {
          const p = typeof res === "string" ? JSON.parse(res) : res;
          if (p && p.success && p.dataUrl) logoImg.src = p.dataUrl;
        });
      } catch(_) {}
      ...
      heroCard.appendChild(logoImg);
      heroCard.appendChild(heroInfo);
      aboutWrap.appendChild(heroCard);
      ...
      function createStatCard(title, value, icon, iconColor) {
        ...
        card.style.cssText = `background:rgba(${c.rgbString},0.05);border:1px solid ${c.borderRgba};border-radius:10px;padding:14px;text-align:center;display:flex;flex-direction:column;gap:6px;`;
        card.innerHTML = `
          <i class="fa-solid ${icon}" style="font-size:20px;color:${iconColor || c.accent};margin-bottom:2px;"></i>
          <span style="font-size:16px;font-weight:700;color:${c.text};">${value}</span>
          <span style="font-size:12px;color:${c.textSecondary};">${title}</span>
        `;
        return card;
      }

      statsGrid.appendChild(createStatCard("Status do Backend", "Ativo & Conectado", "fa-circle-check", "#4ade80"));
      statsGrid.appendChild(createStatCard("Jogos na Base", "+5.800 títulos", "fa-gamepad", c.accent));
      statsGrid.appendChild(createStatCard("Integrações", "OnlineFix & AIO", "fa-bolt", "#fbbf24"));
      aboutWrap.appendChild(statsGrid);

      // Quick Actions Card
      const actionsCard = document.createElement("div");
      actionsCard.style.cssText = `background:rgba(${c.rgbString},0.04);border:1px solid ${c.borderRgba};border-radius:12px;padding:18px;`;
      const actionsTitle = document.createElement("div");
      actionsTitle.style.cssText = `font-size:14px;font-weight:600;color:${c.text};margin-bottom:14px;display:flex;align-items:center;gap:8px;`;
      actionsTitle.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles" style="color:${c.accent};"></i><span>Ações e Diagnósticos do Sistema</span>`;
      actionsCard.appendChild(actionsTitle);

      const actGrid = document.createElement("div");
      actGrid.style.cssText = "display:grid;grid-template-columns:1fr 1fr;gap:10px;";

      function createActionButton(label, icon, onClick) {
        const btn = document.createElement("a");
        btn.href = "#";
        btn.style.cssText = `display:flex;align-items:center;gap:10px;padding:12px 14px;background:rgba(${c.rgbString},0.08);border:1px solid ${c.borderRgba};border-radius:8px;color:${c.text};font-size:13px;font-weight:500;text-decoration:none;transition:all 0.2s ease;cursor:pointer;`;
        btn.innerHTML = `<i class="fa-solid ${icon}" style="color:${c.accent};font-size:16px;width:18px;text-align:center;"></i><span>${label}</span>`;
        btn.onmouseover = function() {
          this.style.background = `rgba(${c.rgbString},0.18)`;
          this.style.borderColor = c.accent;
          this.style.transform = "translateY(-1px)";
      ...
      actGrid.appendChild(createActionButton("Reiniciar Steam", "fa-rotate-right", function() {
      ...
      aboutWrap.appendChild(actionsCard);

      container.appendChild(aboutWrap);
    }
    ...
    saveBtn.addEventListener("click", function (e) {
      ...
      try {
        backendLog(
          "ManifestAdvTools: collectChanges payload " + JSON.stringify(changes),
        );
      } catch (_) {}
      ...
      try {
        backendLog(
          "ManifestAdvTools: sending settings payload " + JSON.stringify(payloadToSend),
        );
      } catch (_) {}
      // Pass flattened keys so Millennium handles the RPC arguments as expected.
      Millennium.callServerMethod("ManifestAdvTools", "ApplySettingsChanges", {
        contentScriptQuery: "",
  ...
  function showLuaToolsAlert(title, message, onClose) {
    ...
    titleEl.textContent = String(title || "ManifestAdvTools");
  ...
  function ShowLuaToolsAlert(title, message) {
    try {
      ...
      backendLog("ManifestAdvTools: Alert error, falling back: " + err);
      try {
        alert(String(title) + "\n\n" + String(message));
      } catch (_) {}
    }
  ...
  function updateButtonTranslations() {
    try {
      ...
      if (restartBtn) {
        const restartText = lt("🔄 Reiniciar Steam");
      ...
      backendLog("ManifestAdvTools: updateButtonTranslations error: " + err);
    }
  }
  ...
  /* ADV_TOOLS_BUNDLE_DL_INJECTED */
  // ── ADV Tools: Manifest + OnlineFix Bundle Download ──────────────────────
  function startBundleDownload(appid, manifestUrl, apiName, installPath) {
    return Millennium.callServerMethod("ManifestAdvTools", "StartAddWithOnlineFix", {
      contentScriptQuery: "",
      ...
      try { return typeof res === "string" ? JSON.parse(res) : res; } catch (_) { return {}; }
    });
  }

  function getBundleStatus(appid) {
    return Millennium.callServerMethod("ManifestAdvTools", "GetBundleStatus", {
      contentScriptQuery: "",
      appid: appid,
    }).then(function (res) {
      try { return typeof res === "string" ? JSON.parse(res) : res; } catch (_) { return {}; }
  ...
  // Poll bundle download until done/failed; calls onProgress(status) and onDone(state)
  function pollBundleUntilDone(appid, onProgress, onDone) {
    var maxPolls = 600; // 5 min max (500ms intervals)
    var polls = 0;
    function tick() {
      if (polls++ > maxPolls) { onDone({ status: "failed", error: "timeout" }); return; }
      getBundleStatus(appid).then(function (res) {
        var state = (res && res.state) ? res.state : { status: "downloading" };
  ...
  // Shows the "Add + OnlineFix" modal and starts the bundle download
  function showBundleDownloadModal(appid, manifestUrl, apiName, installPath) {
    ...
    overlay.style.cssText = [
      ...
      "background:rgba(" + theme.rgbString + ",0.2);backdrop-filter:blur(12px);"
    ].join("");
    ...
    modal.style.cssText = [
      ...
      "border:1px solid rgba(" + theme.rgbString + ",0.32);border-radius:18px;",
      "padding:32px 36px;width:420px;max-width:90vw;",
      "box-shadow:0 12px 48px rgba(0,0,0,0.65), 0 0 28px rgba(" + theme.rgbString + ",0.25);",
      "font-family:'Inter','Motiva Sans',sans-serif;color:" + theme.text + ";"
    ].join("");

    // Header
    var header = document.createElement("div");
    header.style.cssText = "display:flex;align-items:center;gap:14px;margin-bottom:22px;";

    var headerIcon = document.createElement("div");
    headerIcon.style.cssText = [
      "width:42px;height:42px;border-radius:12px;display:flex;align-items:center;justify-content:center;",
      "background:" + theme.gradient + ";",
      "box-shadow:0 0 16px rgba(" + theme.rgbString + ",0.45);font-size:20px;flex-shrink:0;color:#fff;"
    ].join("");
    ...
    cardsWrap.style.cssText = "display:flex;flex-direction:column;gap:10px;margin-bottom:22px;";

    function makeCard(label, id) {
      var card = document.createElement("div");
      card.style.cssText = "background:" + theme.bgTertiary + ";border:1px solid rgba(" + theme.rgbString + ",0.2);border-radius:10px;padding:12px 16px;display:flex;align-items:center;gap:12px;";
      var dot = document.createElement("div");
      dot.id = id + "-dot";
      dot.style.cssText = "width:10px;height:10px;border-radius:50%;background:" + theme.textSecondary + ";flex-shrink:0;transition:background 0.3s;";
      var lbl = document.createElement("span");
      lbl.style.cssText = "font-size:13px;font-weight:500;color:" + theme.text + ";flex:1;";
      lbl.textContent = label;
      var stat = document.createElement("span");
      stat.id = id + "-text";
      stat.style.cssText = "font-size:12px;color:" + theme.textSecondary + ";";
      stat.textContent = lt("Waiting…", "Waiting…");
    ...
    cardsWrap.appendChild(makeCard("📦 " + lt("Manifest", "Manifest"), "adv-manifest"));
    cardsWrap.appendChild(makeCard("🔧 " + lt("OnlineFix", "OnlineFix"), "adv-onlinefix"));

    // Progress bar
    var progressWrap = document.createElement("div");
    progressWrap.style.cssText = "background:rgba(0,0,0,0.35);border-radius:6px;height:6px;overflow:hidden;margin-bottom:22px;border:1px solid rgba(" + theme.rgbString + ",0.12);";
    var progressBar = document.createElement("div");
    progressBar.id = "adv-bundle-progress";
    progressBar.style.cssText = "height:100%;width:5%;border-radius:6px;background:" + theme.gradient + ";transition:width 0.4s ease;";
    progressWrap.appendChild(progressBar);

    // Status label
    var statusLabel = document.createElement("div");
    statusLabel.id = "adv-bundle-status-label";
    statusLabel.style.cssText = "text-align:center;font-size:13px;color:" + theme.textSecondary + ";margin-bottom:20px;";
    statusLabel.textContent = lt("Starting download…", "Starting download…");

    // Close button (disabled until done)
    var closeBtn = document.createElement("button");
    closeBtn.style.cssText = [
      "width:100%;padding:11px;border-radius:10px;border:none;cursor:not-allowed;",
      "background:rgba(255,255,255,0.08);color:" + theme.textSecondary + ";font-size:14px;font-weight:600;",
      "transition:all 0.2s ease;"
    ].join("");
    closeBtn.textContent = lt("Please wait…", "Please wait…");
    ...
    function setCardState(id, status) {
      ...
      if (!dot || !text) return;
      var colors = {
        downloading: theme.accentLight || theme.accent,
        extracting: "#f59e0b",
        done: "#34d399",
        failed: "#f87171",
      ...
      var labels = {
        downloading: lt("Baixando…", "Downloading…"),
        extracting: lt("Instalando…", "Extracting…"),
        done: "✓ " + lt("Concluído", "Done"),
        failed: "✗ " + lt("Falhou", "Failed"),
        not_available: "⚠️ " + lt("Não disponível", "Not available")
      };
      dot.style.background = colors[status] || "#b8b5d8";
      text.textContent = labels[status] || status;
    }

    function setProgress(pct, label) {
      var bar = document.getElementById("adv-bundle-progress");
      var lbl = document.getElementById("adv-bundle-status-label");
      if (bar) bar.style.width = Math.min(100, Math.max(5, pct)) + "%";
      if (lbl) lbl.textContent = label || "";
    }

    function executeBundleDownload(mUrl, aName) {
      startBundleDownload(appid, mUrl, aName, installPath).then(function (res) {
        if (!res || res.success === false) {
          setProgress(100, "✗ " + (res && res.error ? res.error : lt("Falha ao iniciar", "Failed to start")));
          closeBtn.disabled = false;
          closeBtn.style.cssText = "width:100%;padding:11px;border-radius:10px;border:none;cursor:pointer;background:" + theme.gradient + ";color:#fff;font-size:14px;font-weight:600;";
          closeBtn.textContent = lt("Fechar", "Close");
          return;
        }

        setCardState("adv-manifest", "downloading");
        setCardState("adv-onlinefix", "downloading");
        setProgress(15, lt("Baixando manifesto…", "Downloading manifest…"));

        pollBundleUntilDone(appid, function (state) {
          setCardState("adv-manifest", state.manifest || "downloading");
          setCardState("adv-onlinefix", state.fix || "downloading");

          var ms = state.manifest || "downloading";
          var fs = state.fix || "downloading";
          var pct = 20;
          if (ms === "extracting" || fs === "extracting") pct = 60;
          if (ms === "done" && fs !== "done" && fs !== "not_available") pct = 80;
          if (ms === "done" && (fs === "done" || fs === "not_available")) pct = 100;

          var label = lt("Baixando…", "Downloading…");
          if (ms === "extracting") label = lt("Instalando arquivos do jogo…", "Installing game files…");
          if (ms === "done") {
            if (fs === "done") {
          ...
          if (state.status === "failed") label = "✗ " + lt("Falha no download", "Download failed");
          setProgress(pct, label);

        }, function (finalState) {
          closeBtn.disabled = false;
          if (finalState.status === "done") {
            closeBtn.style.cssText = "width:100%;padding:11px;border-radius:10px;border:none;cursor:pointer;background:linear-gradient(135deg,#059669,#34d399);color:#fff;font-size:14px;font-weight:600;";
            closeBtn.textContent = "✓ " + lt("Concluir — Fechar", "Done — Close");
            statusLabel.style.color = "#34d399";
          } else {
            closeBtn.style.cssText = "width:100%;padding:11px;border-radius:10px;border:none;cursor:pointer;background:rgba(239,68,68,0.2);border:1px solid rgba(239,68,68,0.4);color:#f87171;font-size:14px;font-weight:600;";
            closeBtn.textContent = lt("Fechar", "Close");
          }
        });
      }).catch(function(err) {
        setProgress(100, "✗ " + String(err));
        closeBtn.disabled = false;
        closeBtn.textContent = lt("Fechar", "Close");
      });
    }

    // Auto-resolve manifest URL if not pre-cached
    if (!manifestUrl || manifestUrl === "") {
      setProgress(5, lt("Buscando fontes disponíveis…", "Checking available sources…"));
      Millennium.callServerMethod("ManifestAdvTools", "CheckApisForApp", {
        appid: appid,
        contentScriptQuery: "",
      }).then(function (chkRes) {
        try {
          var payload = typeof chkRes === "string" ? JSON.parse(chkRes) : chkRes;
          var results = (payload && payload.results) || [];
          var available = results.filter(function (r) { return r.available; });
          if (available.length > 0) {
            var selected = available[0];
            window.__LuaToolsLastManifestUrl = selected.url;
            window.__LuaToolsLastManifestApi = selected.name;
            executeBundleDownload(selected.url, selected.name);
          } else {
      ...
      executeBundleDownload(manifestUrl, apiName);
    }
  }
  // ─────────────────────────────────────────────────────────────────────────────
function addLuaToolsButton() {
    ...
    if (targetContainer) {
      ...
      if (!existingBtn && !window.__LuaToolsButtonInserted) {
        ...
        // ── ADV Tools: "Add + OnlineFix" bundle button ────────────────────────
        const bundleBtn = document.createElement("a");
        bundleBtn.href = "#";
        bundleBtn.className = (referenceBtn && referenceBtn.className
          ? referenceBtn.className + " luatools-button luatools-bundle-btn"
          : "btnv6_blue_hoverfade btn_medium luatools-button luatools-bundle-btn");
        ...
        try {
          if (referenceBtn) {
            const cs = window.getComputedStyle(referenceBtn);
            bundleBtn.style.marginRight = cs.marginRight;
          }
        } catch (_) {}
        bundleBtn.addEventListener("click", function (e) {
          ...
          try {
            ...
            const clickAppid = urlMatch ? parseInt(urlMatch[1], 10) : window.__LuaToolsCurrentAppId || NaN;
            if (isNaN(clickAppid)) {
              backendLog("Bundle btn: could not determine appid");
              return;
            }
            // Get the best available manifest URL from CheckApisForApp result cache
            // or start from the first enabled API
            var bestUrl = window.__LuaToolsLastManifestUrl || null;
            var bestApi = window.__LuaToolsLastManifestApi || "Unknown";
            // Get game install path
            var installPath = window.__LuaToolsLastInstallPath || "";
            showBundleDownloadModal(clickAppid, bestUrl, bestApi, installPath);
          } catch (bundleErr) {
            backendLog("Bundle btn error: " + bundleErr);
          }
        });
        // ─────────────────────────────────────────────────────────────────────
            ...
            Millennium.callServerMethod("ManifestAdvTools", "HasLuaToolsForApp", {
              ...
              try {
                ...
                if (
                  ...
                  // Insert after restart button (order: Restart → Add → Bundle)
                ...
                fetchFixes(appid).then(function (fixesData) {
                  ...
                  if (btn) {
                    ...
                    if (span && span.textContent === "Unplayable") {
                      span.textContent = lt("Add via ADV Tools");
                    }
                  }
      ...
      if (!logState.missingOnce) {
        backendLog("ManifestAdvTools: steamdbContainer not found on this page");
  ...
  function onFrontendReady() {
    ...
    setTimeout(function () {
      if (
        ...
        backendLog("[ADV Tools] Gamepad detected - Navigation enabled");
    ...
    try {
      if (
        ...
        Millennium.callServerMethod("ManifestAdvTools", "GetInitApisMessage", {
          contentScriptQuery: "",
        }).then(function (res) {
          try {
            const payload = typeof res === "string" ? JSON.parse(res) : res;
        ...
        try {
          if (window.location.hostname === "store.steampowered.com") {
            if (!sessionStorage.getItem("LuaToolsLoadedAppsGate")) {
              ...
              Millennium.callServerMethod("ManifestAdvTools", "ReadLoadedApps", {
                contentScriptQuery: "",
              }).then(function (res) {
                try {
                  const payload =
        ...
        try {
          ...
          if (
            ...
            if (runState.inProgress && runState.appid === appid) {
              backendLog(
                "ManifestAdvTools: operation already in progress for this appid",
              );
              return;
            }
            ...
            const continueWithAdd = function () {
              ...
              Millennium.callServerMethod("ManifestAdvTools", "CheckApisForApp", {
                ...
                .then(function (res) {
                  try {
                    ...
                    if (isFastDownload) {
                      ...
                      backendLog(
                        "ManifestAdvTools: Auto-selecting source via fast download: " + source.name,
                      );
                    ...
                    backendLog("ManifestAdvTools: CheckApisForApp error: " + err);
                    if (status)
                ...
                .catch(function (err) {
                  backendLog("ManifestAdvTools: CheckApisForApp promise error: " + err);
                });
            };
              ...
              const performDownload = function () {
                ...
                const onFailedCallback = function (errMsg) {
                  if (index + 1 < availableSources.length) {
                    backendLog(
                      "ManifestAdvTools: Fast download failed on " +
                        apiName +
                        " (" +
                        errMsg +
                        "). Trying next API: " +
              ...
              if (apiName && apiName.toLowerCase().includes("morrenus")) {
                ...
                if (hubcapKey && /^smm_[0-9a-f]{96}$/.test(hubcapKey)) {
                  ...
                  Millennium.callServerMethod("ManifestAdvTools", "GetMorrenusStats", {
                    ...
                    .catch((e) => {
                      backendLog(
                        "ManifestAdvTools: Error checking Morrenus API limit: " + e,
                      );
  ...
  function startPolling(appid, onFailedCallback) {
    ...
    const timer = setInterval(() => {
      ...
      try {
        Millennium.callServerMethod("ManifestAdvTools", "GetAddViaLuaToolsStatus", {
          appid,
          contentScriptQuery: "",
        }).then(function (res) {
          try {
            ...
            if (st.status === "checking" && st.currentApi && title) {
              ...
              title.textContent = t("common.appName", "ManifestAdvTools");
            }
            ...
            if (["checking content", "done"].includes(st.status)) {
              ...
              if (title) title.textContent = t("common.appName", "ManifestAdvTools");
  ...
  function bootSettings() {
    ...
    loadThemes().then(function() {
        ...
        try { backendLog("ManifestAdvTools: Boot sequence failed: " + String(e)); } catch(_) {}
    });
  }
  bootSettings();
  ...
  function showLoadedAppsPopup(apps) {
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(12px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;";
    const modal = document.createElement("div");
    const loadedAppsModalColors = getThemeColors();
    modal.style.cssText = `background:${loadedAppsModalColors.modalBg};color:${loadedAppsModalColors.text};border:1px solid ${loadedAppsModalColors.border};border-radius:16px;width:560px;padding:28px 32px;box-shadow:0 24px 80px rgba(0,0,0,.65), 0 0 0 1px ${loadedAppsModalColors.shadowRgba};animation:slideUp 0.12s ease-out;`;
    const title = document.createElement("div");
    ...
    dismissBtn.onclick = function (e) {
      e.preventDefault();
      try {
        Millennium.callServerMethod("ManifestAdvTools", "DismissLoadedApps", {
          contentScriptQuery: "",
        });
      } catch (_) {}
      try {
